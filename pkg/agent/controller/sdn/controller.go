...
var (
	sdnControllerPortExternalIDs = map[string]interface{}{
		interfacestore.AntreaInterfaceTypeKey: interfacestore.AntreaTrafficControl,
	}
)

// sdnControllerState keeps the actual state of a TrafficControl that has been realized.
type sdnControllerState struct {
...
type portToTCBinding struct {
	...
	sdnControllers sets.Set[string]
}
...
type Controller struct {
	...
	tcStates      map[string]*sdnControllerState
	tcStatesMutex sync.RWMutex

	sdnControllerInformer     cache.SharedIndexInformer
	sdnControllerLister       crdlisters.TrafficControlLister
	sdnControllerListerSynced cache.InformerSynced
...
func NewTrafficControlController(ofClient openflow.Client,
	...
	c := &Controller{
		...
		sdnControllerInformer:     tcInformer.Informer(),
		sdnControllerLister:       tcInformer.Lister(),
		sdnControllerListerSynced: tcInformer.Informer().HasSynced,
		...
		portToTCBindings:           map[string]*portToTCBinding{},
		tcStates:                   map[string]*sdnControllerState{},
		queue:                      workqueue.NewNamedRateLimitingQueue(workqueue.NewItemExponentialFailureRateLimiter(minRetryDelay, maxRetryDelay), "sdnControllerGroup"),
	}
	c.sdnControllerInformer.AddEventHandlerWithResyncPeriod(
...
func (c *Controller) filterAffectedTCsByPod(pod *v1.Pod) sets.Set[string] {
	...
	allTCs, _ := c.sdnControllerLister.List(labels.Everything())
...
func (c *Controller) Run(stopCh <-chan struct{}) {
	...
	if !cache.WaitForNamedCacheSync(controllerName, stopCh, c.sdnControllerListerSynced, c.podListerSynced, c.namespaceListerSynced) {
		return
	}
...
func (c *Controller) newTrafficControlState(tcName string, action v1alpha2.TrafficControlAction, direction v1alpha2.Direction) *sdnControllerState {
	c.tcStatesMutex.Lock()
	defer c.tcStatesMutex.Unlock()
	state := &sdnControllerState{
...
func (c *Controller) getTrafficControlState(tcName string) (*sdnControllerState, bool) {
...
func (c *Controller) createOVSInternalPort(portName string) (string, error) {
	portUUID, err := c.ovsBridgeClient.CreateInternalPort(portName, 0, "", sdnControllerPortExternalIDs)
	if err != nil {
		return "", err
	}
...
func (c *Controller) getOrCreateTrafficControlPort(port *v1alpha2.TrafficControlPort, portName, tcName string, isReturnPort bool) (uint32, error) {
	...
	if binding, exists := c.portToTCBindings[portName]; exists {
		c.portToTCBindings[portName].sdnControllers.Insert(tcName)
	...
	if itf, ok := c.interfaceStore.GetInterfaceByName(portName); ok {
		...
		c.portToTCBindings[portName] = &portToTCBinding{
			interfaceConfig: itf,
			sdnControllers: sets.New[string](tcName),
		}
	...
	case port.Device != nil:
		portUUID, err = c.ovsBridgeClient.CreatePort(portName, portName, sdnControllerPortExternalIDs)
	case port.VXLAN != nil:
...
func (c *Controller) releaseTrafficControlPort(portName, tcName string, isReturnPort bool) error {
	...
	portBinding.sdnControllers.Delete(tcName)
	if len(portBinding.sdnControllers) == 0 {
...
func (c *Controller) syncTrafficControl(tcName string) error {
	...
	tc, err := c.sdnControllerLister.Get(tcName)
	if err != nil {
...
func (c *Controller) uninstallTrafficControl(tcName string, tcState *sdnControllerState) error {
