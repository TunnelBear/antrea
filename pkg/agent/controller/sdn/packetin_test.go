...
func TestParsePacketIn(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tfc := newFakeSDNController(t, []runtime.Object{tt.expectedTf}, tt.networkConfig, tt.nodeConfig)
			stopCh := make(chan struct{})
			defer close(stopCh)
			tfc.crdInformerFactory.Start(stopCh)
			tfc.crdInformerFactory.WaitForCacheSync(stopCh)
			tfc.runningSDNs[tt.expectedTf.Status.DataplaneTag] = tt.tfState
...
func TestParsePacketInLiveDuplicates(t *testing.T) {
	...
	tfc := newFakeSDNController(t, nil, networkConfig, nodeConfig)
	tfc.runningSDNs[tfState.tag] = tfState

	_, _, _, err := tfc.parsePacketIn(pktIn)
	assert.ErrorIs(t, err, skipSDNUpdateErr)
}
