...
func newFakeSDNController(t *testing.T, initObjects []runtime.Object, networkConfig *config.NetworkConfig, nodeConfig *config.NodeConfig) *fakeSDNController {
	...
	sdnInformer := crdInformerFactory.Crd().V1beta1().SDNs()
	...
	tfController := &Controller{
		...
		crdClient:             crdClient,
		sdnInformer:     sdnInformer,
		sdnLister:       sdnInformer.Lister(),
		sdnListerSynced: sdnInformer.Informer().HasSynced,
		...
		queue:                 workqueue.NewNamedRateLimitingQueue(workqueue.NewItemExponentialFailureRateLimiter(minRetryDelay, maxRetryDelay), "sdn"),
		runningSDNs:     make(map[int8]*sdnState),
	}

	return &fakeSDNController{
...
func TestPreparePacket(t *testing.T) {
	tcs := []struct {
		...
		{
			name: "receive only from a source IPv4 to destination Pod1 in live traffic sdn",
			tf: &crdv1beta1.SDN{
		...
		{
			name: "destination Pod without IPv4 address in live traffic sdn",
			tf: &crdv1beta1.SDN{
		...
		{
			name: "destination Pod without IPv6 address in live traffic sdn",
			tf: &crdv1beta1.SDN{
		...
		{
			name: "Pod-to-IPv6 liveTraffic sdn",
			tf: &crdv1beta1.SDN{
	...
	for _, tt := range tcs {
		t.Run(tt.name, func(t *testing.T) {
			tfc := newFakeSDNController(t, []runtime.Object{tt.tf}, nil, nil)
...
func TestErrSDNCRD(t *testing.T) {
	tf := &crdv1beta1.SDN{
		ObjectMeta: metav1.ObjectMeta{
			Name: "tf",
			UID:  "uid",
	...
	tfc := newFakeSDNController(t, []runtime.Object{tf}, nil, nil)

	gotTf, err := tfc.errorSDNCRD(tf, reason)
	require.NoError(t, err)
	assert.Equal(t, expectedTf, gotTf)
}

func TestStartSDN(t *testing.T) {
	tcs := []struct {
		name           string
		tf             *crdv1beta1.SDN
		ofPort         uint32
	...
	}{
		{
			...
			expectedCalls: func(mockOFClient *openflowtest.MockClient) {
				mockOFClient.EXPECT().InstallSDNFlows(uint8(1), false, false, false, nil, ofPortPod1, uint16(crdv1beta1.DefaultSDNTimeout))
				mockOFClient.EXPECT().SendSDNPacket(uint8(1), &binding.Packet{
					SourceIP:       net.ParseIP(pod1IPv4),
					SourceMAC:      pod1MAC,
					DestinationIP:  net.ParseIP(pod2IPv4),
					DestinationMAC: pod2MAC,
		...
		},
		{
			...
			ofPort: ofPortPod2,
			expectedCalls: func(mockOFClient *openflowtest.MockClient) {
				mockOFClient.EXPECT().InstallSDNFlows(uint8(1), true, false, true, &binding.Packet{DestinationMAC: pod2MAC}, ofPortPod2, uint16(crdv1beta1.DefaultSDNTimeout))
			},
		},
	}
	...
	for _, tt := range tcs {
		t.Run(tt.name, func(t *testing.T) {
			tfc := newFakeSDNController(t, []runtime.Object{tt.tf}, nil, tt.nodeConfig)
...
func TestSyncSDN(t *testing.T) {
	tcs := []struct {
		name          string
		tf            *crdv1beta1.SDN
		existingState *sdnState
		newState      *sdnState
		expectedCalls func(mockOFClient *openflowtest.MockClient)
	}{
		{
			name: "sdn in running phase",
			tf: &crdv1beta1.SDN{
				ObjectMeta: metav1.ObjectMeta{Name: "tf1", UID: "uid1"},
				Spec: crdv1beta1.SDNSpec{
					Source: crdv1beta1.Source{
		...
		{
			name: "sdn in running phase with empty state",
			tf: &crdv1beta1.SDN{
				ObjectMeta: metav1.ObjectMeta{Name: "tf1", UID: "uid1"},
				Spec: crdv1beta1.SDNSpec{
					Source: crdv1beta1.Source{
			...
			expectedCalls: func(mockOFClient *openflowtest.MockClient) {
				mockOFClient.EXPECT().InstallSDNFlows(uint8(1), false, false, false, nil, uint32(1), uint16(20))
				mockOFClient.EXPECT().SendSDNPacket(uint8(1), gomock.Any(), ofPortPod1, int32(-1))
			},
		},
		{
			name: "sdn in running phase with conflict state",
			tf: &crdv1beta1.SDN{
				ObjectMeta: metav1.ObjectMeta{Name: "tf1", UID: "uid1"},
				Spec: crdv1beta1.SDNSpec{
					Source: crdv1beta1.Source{
			...
			expectedCalls: func(mockOFClient *openflowtest.MockClient) {
				mockOFClient.EXPECT().UninstallSDNFlows(uint8(1))
				mockOFClient.EXPECT().InstallSDNFlows(uint8(1), false, false, false, nil, uint32(1), uint16(20))
				mockOFClient.EXPECT().SendSDNPacket(uint8(1), gomock.Any(), ofPortPod1, int32(-1))
			},
		},
		{
			name: "sdn in failed phase",
			tf: &crdv1beta1.SDN{
				ObjectMeta: metav1.ObjectMeta{Name: "tf1", UID: "uid1"},
				Spec: crdv1beta1.SDNSpec{
					Source: crdv1beta1.Source{
	...
	for _, tt := range tcs {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.existingState != nil {
				tfc.runningSDNs[tt.tf.Status.DataplaneTag] = tt.existingState
			}
			...
			err := tfc.syncSDN(tt.tf.Name)
			require.NoError(t, err)
			assert.Equal(t, tt.newState, tfc.runningSDNs[tt.tf.Status.DataplaneTag])
		})
	}
}

func TestProcessSDNItem(t *testing.T) {
	tc := struct {
		tf           *crdv1beta1.SDN
		ofPort       uint32
		receiverOnly bool
	...
	tfc := newFakeSDNController(t, []runtime.Object{tc.tf}, nil, nil)
	stopCh := make(chan struct{})
	defer close(stopCh)
	tfc.crdInformerFactory.Start(stopCh)
	tfc.crdInformerFactory.WaitForCacheSync(stopCh)

	tfc.mockOFClient.EXPECT().InstallSDNFlows(uint8(tc.tf.Status.DataplaneTag), tc.tf.Spec.LiveTraffic, tc.tf.Spec.DroppedOnly, tc.receiverOnly, nil, tc.ofPort, uint16(crdv1beta1.DefaultSDNTimeout))
	tfc.mockOFClient.EXPECT().SendSDNPacket(uint8(tc.tf.Status.DataplaneTag), tc.packet, tc.ofPort, int32(-1))
	tfc.enqueueSDN(tc.tf)
	got := tfc.processSDNItem()
	assert.Equal(t, tc.expected, got)
}

func TestValidateSDN(t *testing.T) {
	tcs := []struct {
		name               string
		tf                 *crdv1beta1.SDN
